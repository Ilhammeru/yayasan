...
(() => {
/*!************************************!*\
  !*** ./resources/js/mainIncome.js ***!
  \************************************/
var _require = __webpack_require__(/*! tippy.js */ "./node_modules/tippy.js/dist/tippy.esm.js"),
  tippy = _require["default"];
$('#user').chosen({
  width: '100%'
});
...
function getDetailUser(e) {
  ...
  $.ajax({
    ...
    success: function success(res) {
      ...
      for (var a = 0; a < res.data.institutions.length; a++) {
        option += "<option value=\"".concat(res.data.institutions[a].id, "\" ").concat(res.data.institutions[a].selected, ">\n            \t\t\t").concat(res.data.institutions[a].name, "\n            \t\t</option>");
      }
...
function addItem(incomeCategoryId, isEnable) {
  ...
  $.ajax({
    ...
    success: function success(res) {
      ...
      var deleteElem = $('.td-delete');
      for (var a = 0; a < deleteElem.length; a++) {
        deleteElem[a].classList.remove('d-none');
      }
      $('.th-additional').removeClass('d-none');

      // init select2
      var select2add = $('.select-chosen').select2();
      ...
      $('.select-month').MonthPicker({
        ...
        Button: function Button(options) {
          ...
          return $("<span class=\"input-group-addon\" style=\"display: table-cell !important;\"><i class=\"fa fa-calendar\"></i></span>").button({
            text: true,
...
function deleteRow(id) {
  ...
  if (tr.length == 2) {
    ...
    var lastElem = $('.tr-item');
    var lastId = lastElem[0].id;
    var lastKey = $('#' + lastId).data('key');
...
function updateTotal(e) {
  ...
  var inputs = $('.price_item_shadow');
  for (var a = 0; a < inputs.length; a++) {
    var arrayVal = inputs[a].value;
    val.push(arrayVal);
  }
  // sum all value
  var sum = val.reduce(function (partialSum, a) {
    return parseInt(partialSum) + parseInt(a);
  }, 0);

  // manipulate views
  var res = numberWithCommas(sum);
  $('#amount_total').val(sum);
  $('#amount_total_shadow').val(res);
  $('#remaining_bill').val(res);
}
...
function saveItem() {
  ...
  var data = new FormData($('#form-invoice')[0]);
  var ckEditor = CKEDITOR.instances.ckeditor_message.getData();
...
function appendPaymentDetail(id) {
  var totalItem = $('#total-item-td');
  var remaining = $('#remaining-bill-td');
  $.ajax({
    type: "POST",
...
function pay(id) {
  ...
  var data = new FormData($('#form-payment')[0]);
  $.ajax({
    type: "POST",
...
function normalizeValue(e) {
  var val = e.value.toString().replace(',', '').replace(',', '').replace(',', '');
  e.value = val;
}
...
function detailInvoiceMonthly(institutionId, classId, levelId, userId, month, incomeCategoryId, incomeTypeId, incomeCategoryName) {
  var payload = {
  ...
  openModalInvoice(base_url + '/incomes/invoice/monthly/form', payload, 'payInvoiceMonthly()', true, i18n.view.invoice + ' ' + incomeCategoryName);
}
function detailPaidInvoice(paymentId) {
  openModalInvoice(base_url + "/incomes/".concat(paymentId), null, "printPaidInvoice(".concat(paymentId, ")"), true, i18n.view.detail_invoice, i18n.view.close, i18n.view.print);
}
function printPaidInvoice(paymentId) {
  window.open(base_url + '/incomes/' + paymentId + '?print=true', '_blank');
}
function createInvoiceNonPeriod(institutionId, classId, levelId, incomeCategoryId, incomeTypeId, incomeCategoryName) {
  var payload = {
    institution_id: institutionId,
    class_id: classId,
    level_id: levelId,
  ...
  openModalInvoice(base_url + '/incomes/invoice-non-period', payload, 'saveInvoiceNonPeriod()', i18n.view.create_invoice);
}
function openModalInvoice(urlToGenerateBody) {
  var payloadToGenerateBody = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : null;
  var actionSave = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : null;
  var fullscreen = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : false;
  var modalTitle = arguments.length > 4 && arguments[4] !== undefined ? arguments[4] : 'Title';
  var closeText = arguments.length > 5 && arguments[5] !== undefined ? arguments[5] : i18n.view.close;
  var saveText = arguments.length > 6 && arguments[6] !== undefined ? arguments[6] : i18n.view.save;
  ...
  $.ajax({
    ...
    success: function success(res) {
      $('#invoice-modal .modal-body').html(res.view).css({
        'display': 'block'
      });
      ...
      if (actionSave) {
        $('#invoice-modal .btn-save').attr('onclick', "".concat(actionSave));
      }
      var d = new Date();
      var year = d.getFullYear();
      /**
...
function changeIncomeByLevel(levelId, classId, institutionId, incomeCategoryId, incomeTypeId, incomeTypePeriod, incomeCategoryName) {
  var payload = {
    level_id: levelId,
    class_id: classId,
    institution_id: institutionId,
...
function previewImage(path) {
  ...
  $('#preview-modal-image .modal-body').html("\n        <img src=\"".concat(path, "\" style=\"width: 100%; height: auto;\" />\n    ")).css({
    'display': 'flex',
    'alignItems': 'center',
    'justifyContent': 'center'
  });
