...
var _require = __webpack_require__(/*! sweetalert2 */ "./node_modules/sweetalert2/dist/sweetalert2.all.js"),
  Swal2 = _require["default"];
var _require2 = __webpack_require__(/*! tippy.js */ "./node_modules/tippy.js/dist/tippy.esm.js"),
  tippy = _require2["default"];
base_url = window.location.origin;
updateUserSaldo();
function openGlobalModal(urlToGenerateBody) {
  ...
  $.ajax({
    ...
    success: function success(res) {
      ...
      $('#global-modal .modal-body').html(res.view).css({
        'display': 'block'
      });
      $('#global-modal .modal-footer').removeClass('d-none');
      if (actionSave) {
        $('#global-modal .btn-save').attr('onclick', "".concat(actionSave));
      }
    }
  });
...
function createDataTables(tableId, columns, route) {
...
function openModalWithValue(method, formId, modalId, modalLabel, textLabel, urlReq) {
  var urlRes = arguments.length > 6 && arguments[6] !== undefined ? arguments[6] : null;
  var needToOpenTab = arguments.length > 7 && arguments[7] !== undefined ? arguments[7] : false;
  $.ajax({
    type: method,
    url: urlReq,
    beforeSend: function beforeSend() {},
    success: function success(res) {
      buildModalBodyGeneral(textLabel, res.url, res.view, res.method, modalLabel, formId, modalId);
      if (needToOpenTab) {
        openTab('general');
      }
    },
...
function buildModalBodyGeneral(text, url, view, method, modalLabel, formId, modalId) {
  $("#".concat(modalLabel)).text(text);
  $("#".concat(formId)).attr('action', base_url + url);
  $("#".concat(formId)).attr('method', method);
  $("#".concat(modalId, " .modal-body")).html(view);
...
function disableButton(id) {
  ...
  var text = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : i18n.view.save_changes;
  var loading = "<i class=\"fa fa-spinner fa fa-spin\"></i>";
  $("#".concat(id)).prop('disabled', isDisable);
...
function loadingPage() {
  ...
  var text = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 'Checking data';
  var loading = Swal2.mixin({
...
function reloadWalletDetail(incomeCategoryId) {
  ...
  createDataTables('table-wallet', columns, dt_route, param);

  /**
   * Manipulate buttons
   */
  var elems = $('.widget-wallet');
...
function openSendingWalletForm() {
  ...
  var filter = maps.filter(Boolean);
  if (filter.length == 0) {
    return showNotif(true, i18n.view.choose_wallet_item);
  }
  val = JSON.stringify(val);
  var incomeCategoryId = $('#wallet-tab-active').data('active');
  $.ajax({
    type: 'POST',
...
function sendWallet() {
  var incomeCategoryId = $('#wallet-tab-active').data('active');
  var form = $('#form-send-wallet');
  var data = new FormData($('#form-send-wallet')[0]);
  var ckEditor = CKEDITOR.instances.ckeditor_message_send_wallet.getData();
